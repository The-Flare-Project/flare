...
USER_PROGRAMS=$(USER_SOURCES:c=fapp)
KERNEL_SOURCES=$(wildcard kernel/*.[chS])

all: basekernel.iso

run: basekernel.iso disk.img
	qemu-system-i386 -cdrom basekernel.iso -hda disk.img -serial stdio
...
image: kernel/basekernel.img $(USER_PROGRAMS)
	rm -rf image
	mkdir image image/boot image/bin image/data image/source
	cp kernel/basekernel.img image/boot
	# Copy sources
	cp kernel image/source/kernel -r
	cp include image/source/include -r
	cp library image/source/library -r
	cp user image/source/user -r
	cd image && rm -rf *.o *.fapp && cd ..
